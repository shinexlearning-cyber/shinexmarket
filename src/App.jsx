...
import {
  ...
  Eye, EyeOff, ShoppingBag, Bell, Settings, HelpCircle,
  Globe, Lock, ShieldCheck, ChevronRight as ChevronRightIcon, Share2,
  ShoppingCart, Minus, Truck, PackageCheck, XCircle
} from "lucide-react";
...
/* ============================================================
   ...
   - Admin functionality now lives in a separate app (SHINEX Admin) —
     this file intentionally has no /admin/* calls.
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
function AppTopBar({ nav, go, query, setQuery, onSearchSubmit, onOpenDrawer, cartCount }) {
  const { user } = useAuth();
  ...
  return (
    <header className="sticky top-0 z-40 bg-white border-b border-gray-100">
      <div className="max-w-7xl mx-auto px-3 sm:px-5 h-14 flex items-center gap-2 sm:gap-4">
        ...
        <button
          onClick={() => go("cart")}
          className="relative p-2 rounded-lg hover:bg-gray-50 text-gray-700 shrink-0"
          aria-label="Cart"
        >
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto px-2 py-2.5 space-y-0.5">
          ...
          {item("orders", "Orders", Truck, { requiresAuth: true })}
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
function ProductDetailPage({ params, go, favoriteProductIds, toggleFavorite, addToCart }) {
          ...
          <div className="flex gap-3 mt-6">
            {!product.is_sold && !isOwnProduct && (
              <>
                ...
                <Button onClick={() => handleAddToCart(true)} className="flex-1" disabled={addingToCart}>
                  Buy Now
                </Button>
              </>
            )}
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
function GoogleSignInButton({ go }) {
...
/* ------------------------------------------------------------
   PAGE: CART — GET/PUT/DELETE /cart
   Adding to cart does not charge anything — see CheckoutPage for why
   (no Paystack Subaccount/payout path to individual sellers exists).
   ------------------------------------------------------------ */
function CartPage({ go, onCartChange }) {
  const { user } = useAuth();
  const toast = useToast();
  ...
  if (!user) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <EmptyState icon={ShoppingCart} title="Log in to see your cart" action={<Button onClick={() => go("login")}>Log in</Button>} />
      </div>
    );
  }
  ...
  const removeItem = async (id) => {
    ...
    try {
      ...
      onCartChange();
      toast.push("Removed from cart", "success");
    } catch (e) {
      setItems(prev);
      toast.push(e.message, "error");
    }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      {status === "error" && <ErrorState message="Couldn't load your cart." onRetry={load} />}
      {status === "ready" && items.length === 0 && (
        <EmptyState icon={ShoppingCart} title="Your cart is empty" subtitle="Browse the marketplace and add something you like." action={<Button onClick={() => go("home")}>Browse products</Button>} />
      )}
        ...
        <>
          <div className="space-y-3">
            {items.map((item) => {
              ...
              const image = p?.images?.find((im) => im.is_primary)?.image_url || p?.images?.[0]?.image_url;
              return (
                <div key={item.id} className="flex items-center gap-3 bg-white rounded-xl border border-gray-100 p-3">
                  <button onClick={() => p && go("product", { id: p.id })} className="w-16 h-16 rounded-lg overflow-hidden bg-gray-50 shrink-0">
                    {image ? <img src={image} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-gray-300"><ShoppingBag size={20} /></div>}
                  </button>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-800 truncate">{p?.name || "Unavailable listing"}</p>
                    <p className="text-sm font-bold mt-0.5" style={{ color: COLORS.secondary }}>{money(p?.price)}</p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <button onClick={() => updateQty(item.id, item.quantity - 1)} disabled={updatingId === item.id || item.quantity <= 1} className="w-7 h-7 rounded-lg border border-gray-200 flex items-center justify-center disabled:opacity-40">
                      <Minus size={13} />
                    </button>
                    <span className="text-sm font-medium w-5 text-center">{item.quantity}</span>
                    <button onClick={() => updateQty(item.id, item.quantity + 1)} disabled={updatingId === item.id} className="w-7 h-7 rounded-lg border border-gray-200 flex items-center justify-center disabled:opacity-40">
                      <Plus size={13} />
                    </button>
                  </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4 mt-5 flex items-center justify-between">
            <span className="text-sm text-gray-500">Subtotal</span>
            <span className="text-lg font-bold" style={{ color: COLORS.primary }}>{money(subtotal)}</span>
          </div>
          <Button className="w-full !py-3 mt-4" onClick={() => go("checkout")}>Proceed to Checkout</Button>
        </>
      )}
    </div>
  );
...
/* ------------------------------------------------------------
   PAGE: CHECKOUT — POST /orders
   IMPORTANT: this does not process any payment. There is no Paystack
   Subaccount/split-payment configuration for individual sellers and
   products have no stock/quantity concept, so SHINEX cannot honestly
   "take payment" for goods it has no path to pay sellers for. This
   creates a real order record (one per seller in the cart) and the
   buyer/seller then arrange payment directly, same as any other
   listing on SHINEX today.
   ------------------------------------------------------------ */
function CheckoutPage({ go, onOrderPlaced }) {
  const { user } = useAuth();
  const toast = useToast();
  const [items, setItems] = useState([]);
  const [status, setStatus] = useState("loading");
  ...
  if (!user) {
    return <EmptyState icon={Truck} title="Log in to checkout" action={<Button onClick={() => go("login")}>Log in</Button>} />;
  }

  const subtotal = items.reduce((sum, i) => sum + Number(i.product?.price || 0) * i.quantity, 0);
  const sellerCount = new Set(items.map((i) => i.product?.user_id)).size;

  const placeOrder = async () => {
    if (!address.trim()) {
      toast.push("Enter a delivery address.", "error");
      return;
    }
    setPlacing(true);
    try {
      await api("/orders", { method: "POST", body: { delivery_address: address.trim(), notes: notes.trim() || undefined } });
      toast.push("Order placed! The seller will reach out to arrange payment and delivery.", "success");
      onOrderPlaced();
      go("orders");
    } catch (e) {
      toast.push(e.message, "error");
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <p className="text-sm text-gray-500 mb-6">Placing an order sends your details to the seller — you'll arrange payment and delivery directly with them, just like contacting a seller normally.</p>

      {status === "loading" && <Skeleton className="h-40" />}
      {status === "error" && <ErrorState message="Couldn't load your cart." onRetry={load} />}
      {status === "ready" && items.length === 0 && (
        <EmptyState icon={ShoppingCart} title="Your cart is empty" action={<Button onClick={() => go("home")}>Browse products</Button>} />
      )}

      {status === "ready" && items.length > 0 && (
        <>
          <div className="bg-white rounded-xl border border-gray-100 p-4 space-y-3 mb-5">
            {items.map((item) => (
              <div key={item.id} className="flex items-center justify-between text-sm">
                <span className="text-gray-700 truncate pr-3">{item.product?.name} <span className="text-gray-400">× {item.quantity}</span></span>
                <span className="font-medium text-gray-800 shrink-0">{money(Number(item.product?.price || 0) * item.quantity)}</span>
              </div>
            ))}
            <div className="border-t border-gray-100 pt-3 flex items-center justify-between">
              <span className="text-sm font-semibold text-gray-800">Subtotal{sellerCount > 1 ? ` (${sellerCount} sellers)` : ""}</span>
              <span className="font-bold" style={{ color: COLORS.secondary }}>{money(subtotal)}</span>
            </div>
          </div>

          <div className="bg-white rounded-xl border border-gray-100 p-4 space-y-4">
            <TextArea label="Delivery address" rows={3} value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Where should the seller deliver this?" />
            <TextArea label="Notes (optional)" rows={2} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Anything the seller should know" />
          </div>

          <Button className="w-full !py-3 mt-5" onClick={placeOrder} disabled={placing}>
...
/* ------------------------------------------------------------
   PAGE: ORDERS — GET /orders/my (purchases), GET /orders/received (sales)
   ------------------------------------------------------------ */
const ORDER_STATUS_STYLE = {
  pending: { bg: "#FEF3C7", fg: "#B45309", icon: Truck },
  confirmed: { bg: "#DBEAFE", fg: "#2563EB", icon: PackageCheck },
  completed: { bg: "#D1FAE5", fg: "#059669", icon: PackageCheck },
  cancelled: { bg: "#FEE2E2", fg: "#DC2626", icon: XCircle },
};
...
function OrderCard({ order, isSeller, onStatusChange }) {
  ...
  const whatsappNumber = (other?.whatsapp || "").replace(/[^0-9]/g, "");

  return (
    <div className="bg-white rounded-xl border border-gray-100 p-4">
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-semibold px-2 py-1 rounded-md flex items-center gap-1" style={{ backgroundColor: style.bg, color: style.fg }}>
          <style.icon size={11} /> {order.status}
        </span>
        <span className="text-xs text-gray-400">{new Date(order.created_at).toLocaleDateString()}</span>
      </div>
      <div className="space-y-1 mb-2">
        {(order.items || []).map((i) => (
          <p key={i.id} className="text-sm text-gray-700">{i.product_name} <span className="text-gray-400">× {i.quantity}</span></p>
        ))}
      </div>
      <div className="flex items-center justify-between border-t border-gray-50 pt-2">
        <span className="text-xs text-gray-400">{isSeller ? "Buyer" : "Seller"}: {other?.full_name || other?.shop_name || other?.username || "—"}</span>
        <span className="font-bold text-sm" style={{ color: COLORS.secondary }}>{money(order.subtotal)}</span>
      </div>

      <div className="flex flex-wrap gap-2 mt-3">
        {whatsappNumber && (
          <Button as="a" href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noreferrer" variant="outline" className="!bg-white !py-1.5 !px-3 text-xs">
            <MessageCircle size={13} /> WhatsApp
          </Button>
        )}
        {isSeller && order.status === "pending" && (
          <Button onClick={() => setStatus("confirmed")} disabled={updating} className="!py-1.5 !px-3 text-xs">Confirm</Button>
        )}
        {isSeller && order.status === "confirmed" && (
          <Button onClick={() => setStatus("completed")} disabled={updating} variant="secondary" className="!py-1.5 !px-3 text-xs">Mark completed</Button>
        )}
        {order.status !== "cancelled" && order.status !== "completed" && (
          <button onClick={() => setStatus("cancelled")} disabled={updating} className="text-xs font-medium text-gray-400 hover:text-red-500 px-2">Cancel</button>
        )}
      </div>
    </div>
  );
...
function OrdersPage({ go }) {
  ...
  const load = useCallback(async () => {
    ...
    setStatus("loading");
    try {
      const { data } = await api(tab === "purchases" ? "/orders/my" : "/orders/received");
      setOrders(data || []);
      setStatus("ready");
    } catch (e) {
      setStatus("error");
  ...
  if (!user) {
    return <EmptyState icon={Truck} title="Log in to see your orders" action={<Button onClick={() => go("login")}>Log in</Button>} />;
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="flex gap-1 bg-white rounded-lg border border-gray-100 p-1 w-fit mb-6">
        {[{ k: "purchases", l: "Purchases" }, { k: "sales", l: "Sales" }].map((t) => (
          <button
            key={t.k}
            onClick={() => setTab(t.k)}
            className="px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors"
      ...
      {status === "loading" && <div className="space-y-3">{Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-32" />)}</div>}
      {status === "error" && <ErrorState message="Couldn't load orders." onRetry={load} />}
      {status === "ready" && orders.length === 0 && (
        <EmptyState
          icon={Truck}
          title={tab === "purchases" ? "No orders yet" : "No sales yet"}
          subtitle={tab === "purchases" ? "Orders you place will show up here." : "Orders buyers place with you will show up here."}
        />
      )}
      {status === "ready" && orders.length > 0 && (
        <div className="space-y-3">
          {orders.map((o) => (
            <OrderCard key={o.id} order={o} isSeller={tab === "sales"} onStatusChange={load} />
          ))}
        </div>
      )}
    </div>
...
function AppShell() {
  ...
  const refreshCartCount = useCallback(async () => {
    ...
    try {
      ...
      // silent — cart badge is non-critical to first paint
    }
  }, [user]);
  ...
  const addToCart = async (productId, quantity = 1) => {
    await api("/cart", { method: "POST", body: { product_id: productId, quantity } });
    refreshCartCount();
  };
  ...
  switch (nav.page) {
    ...
    case "product": content = <ProductDetailPage params={nav.params} go={go} favoriteProductIds={favoriteProductIds} toggleFavorite={toggleFavorite} addToCart={addToCart} />; break;
    ...
    case "cart": content = <CartPage go={go} onCartChange={refreshCartCount} />; break;
    case "checkout": content = <CheckoutPage go={go} onOrderPlaced={refreshCartCount} />; break;
    case "orders": content = <OrdersPage go={go} />; break;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      `}</style>
      <AppTopBar nav={nav} go={go} query={query} setQuery={setQuery} onSearchSubmit={onSearchSubmit} onOpenDrawer={() => setDrawerOpen(true)} cartCount={cartCount} />
