...
/* ------------------------------------------------------------
   PAGE: MY SHOP (owner management view) — GET /products/mine/all,
   DELETE /products/:id. This is deliberately a different component
   from the public ShopPage: it shows pending/rejected listings and
   edit/delete controls that a visitor must never see (Part 6/7/17).
   ------------------------------------------------------------ */
...
function MyShopPage({ go }) {
  ...
  const load = useCallback(async () => {
    setStatus("loading");
    try {
      const { data, plan: planInfo } = await api("/products/mine/all");
      setProducts(data || []);
      setPlan(planInfo || null);
      setStatus("ready");
    } catch (e) {
      setStatus("error");
    }
  ...
  if (!user) {
    return <EmptyState icon={Store} title="Log in to manage your shop" action={<Button onClick={() => go("login")}>Log in</Button>} />;
  }
  ...
  const activeCount = products.filter((p) => p.is_active && ["pending", "approved"].includes(p.approval_status)).length;
  const limitReached = plan?.active_listing_limit !== null && plan?.active_listing_limit !== undefined && activeCount >= plan.active_listing_limit;

  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      <div className="bg-white rounded-2xl border border-gray-100 p-6 flex flex-col sm:flex-row gap-5 items-start">
        <div className="w-20 h-20 rounded-full bg-gray-100 overflow-hidden flex items-center justify-center shrink-0">
          {user.avatar_url ? <img src={user.avatar_url} className="w-full h-full object-cover" alt="" /> : <User size={32} className="text-gray-400" />}
        </div>
        <div className="flex-1">
          <h1 className="text-xl font-bold text-gray-900">{user.shop_name || user.username}</h1>
          ...
          <div className="flex gap-3 mt-3">
            <Button variant="outline" className="!bg-white !py-2" onClick={() => go("shop", { username: user.username })}>
              View public shop
            </Button>
        ...
        <Button onClick={() => go("sell")} disabled={limitReached} className="!py-2.5 shrink-0">
          <Plus size={16} /> Add listing
        </Button>
      </div>

      {plan && (
        <div className="mt-4 rounded-xl px-4 py-3 text-sm flex items-center justify-between flex-wrap gap-2" style={{ backgroundColor: `${COLORS.primary}08` }}>
          <span className="text-gray-600">
            <strong className="capitalize">{plan.name}</strong> plan —{" "}
            {plan.active_listing_limit === null
              ? "unlimited active listings"
              : `${activeCount}/${plan.active_listing_limit} active listings used`}
          </span>
          {plan.name === "starter" && (
            <button onClick={() => go("advertise")} className="font-semibold text-xs" style={{ color: COLORS.primary }}>
      ...
      {limitReached && (
        <div className="mt-3 rounded-xl bg-red-50 border border-red-100 px-4 py-3 text-sm text-red-700">
          You've reached your {plan.active_listing_limit}-listing limit. Delete one of your active listings or subscribe to a SHINEX plan to list more.
        </div>
      )}
      ...
      <div className="flex items-center gap-2 mt-6 mb-4 overflow-x-auto no-scrollbar">
        {[
          ...
          <button
            ...
            style={filter === t.key ? { backgroundColor: COLORS.primary, borderColor: COLORS.primary, color: "#fff" } : { backgroundColor: "#fff", borderColor: "#E5E7EB", color: "#6B7280" }}
          >
            {t.label}
          </button>
        ))}
      ...
      {status === "error" && <ErrorState message="Couldn't load your listings." onRetry={load} />}
      {status === "ready" && filtered.length === 0 && (
        <EmptyState icon={Package} title="No listings here" subtitle="Nothing to show in this filter yet." action={<Button onClick={() => go("sell")}>List a product</Button>} />
      )}
      {status === "ready" && filtered.length > 0 && (
            ...
            <div key={p.id} className="bg-white rounded-2xl border border-gray-100 p-3 flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {p.approval_status === "rejected" && p.rejection_reason && (
                  <p className="text-xs text-red-500 mt-1 truncate" title={p.rejection_reason}>Reason: {p.rejection_reason}</p>
                )}
                <div className="flex items-center gap-3 mt-2">
                  <button onClick={() => go("product", { id: p.id })} className="text-xs font-semibold flex items-center gap-1" style={{ color: COLORS.primary }}>
                    <Eye size={13} /> Preview
                  </button>
                  <button onClick={() => go("sell", { editId: p.id })} className="text-xs font-semibold flex items-center gap-1 text-gray-500">
                    <Edit2 size={13} /> Edit
                  </button>
                  <button onClick={() => remove(p.id)} className="text-xs font-semibold flex items-center gap-1 text-red-500">
...
function RegisterPage({ go }) {
  ...
  const [form, setForm] = useState({
    ...
    accountType: "sell", shopName: "", location: "", whatsapp: ""
  });
  ...
  const isSeller = form.accountType === "sell";
  ...
  const validate = () => {
    ...
    if (isSeller && form.whatsapp && !/^\+?[0-9]{10,15}$/.test(form.whatsapp.replace(/[\s\-()]/g, ""))) {
      e.whatsapp = "Use international format, e.g. +2348012345678";
    }
    setErrors(e);
  ...
  const submit = async (ev) => {
    ...
    try {
      await register({
        ...
        account_type: isSeller ? "sell" : "buy",
        ...(isSeller ? { shop_name: form.shopName, location: form.location, whatsapp: form.whatsapp } : {}),
      });
  ...
  return (
    <AuthLayout title="Create your account" subtitle="Join SHINEX to buy and sell with your community." go={go} switchLabel="Already have an account?" switchCta="Log in" switchTo="login">
      <form onSubmit={submit} className="space-y-4">
        ...
        <div>
          <span className="block text-sm font-medium text-gray-700 mb-2">What do you want to do on SHINEX?</span>
          <div className="grid grid-cols-2 gap-3">
            <button
              type="button"
              onClick={() => setForm((f) => ({ ...f, accountType: "buy" }))}
              className="rounded-xl border-2 px-3 py-3 text-sm font-semibold text-left"
              style={!isSeller ? { borderColor: COLORS.primary, color: COLORS.primary, backgroundColor: `${COLORS.primary}08` } : { borderColor: "#E5E7EB", color: "#6B7280" }}
            >
              I only want to buy
            </button>
            <button
              type="button"
              onClick={() => setForm((f) => ({ ...f, accountType: "sell" }))}
              className="rounded-xl border-2 px-3 py-3 text-sm font-semibold text-left"
              style={isSeller ? { borderColor: COLORS.primary, color: COLORS.primary, backgroundColor: `${COLORS.primary}08` } : { borderColor: "#E5E7EB", color: "#6B7280" }}
            >
              I want to sell
          ...
          <p className="text-xs text-gray-400 mt-2">One SHINEX account either way — you can turn selling on later from your profile.</p>
        </div>

        {isSeller && (
          <div className="space-y-4 rounded-xl bg-gray-50 p-4">
            <Input label="Shop name (optional)" value={form.shopName} onChange={set("shopName")} placeholder="e.g. Amaka's Fashion Store" />
            <Input label="Location" value={form.location} onChange={set("location")} placeholder="e.g. Ikeja, Lagos" />
            <Input label="WhatsApp number" value={form.whatsapp} onChange={set("whatsapp")} error={errors.whatsapp} placeholder="+2348012345678" />
          </div>
        )}
...
function SellPage({ go, params }) {
  ...
  const [images, setImages] = useState([]); // { file?, url, existing? }
  const [errors, setErrors] = useState({});
  const [loading, setLoading] = useState(false);
  ...
  // Edit mode: load the existing listing (ownership is enforced by the
  // backend regardless — this just prefills the form).
  useEffect(() => {
    if (!editId) return;
    (async () => {
      try {
        const { data: product } = await api(`/products/${editId}`);
        setForm({
          name: product.name || "",
        ...
        setImages((product.images || []).map((img) => ({ url: img.image_url, existing: true })));
      } catch (e) {
        toast.push("Couldn't load this listing for editing.", "error");
        go("myshop");
      } finally {
  ...
  const submit = async () => {
    ...
    try {
      ...
      let data;
      if (editId) {
        ({ data } = await api(`/products/${editId}`, { method: "PUT", body: fd, formData: true }));
        toast.push("Listing updated — it's back in the pending review queue.", "success");
      } else {
        ({ data } = await api("/products", { method: "POST", body: fd, formData: true }));
        toast.push("Submitted! Your listing is pending admin review — we'll notify you once it's approved.", "success");
      }
      go("myshop");
    } catch (e) {
  ...
  if (step === "preview") {
    const previewProduct = {
      ...
      seller: { username: user.username, shop_name: user.shop_name, whatsapp: user.whatsapp, full_name: user.full_name },
    };

    return (
      <div className="max-w-2xl mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold text-gray-900">Preview your listing</h1>
        <p className="text-sm text-gray-500 mt-1 mb-6">This is how buyers will see it once it's approved. You can still go back and edit anything.</p>

        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="aspect-square bg-gray-100">
            {previewProduct.primary_image && <img src={previewProduct.primary_image} alt="" className="w-full h-full object-cover" />}
          </div>
          {images.length > 1 && (
            <div className="flex gap-2 p-3 overflow-x-auto no-scrollbar">
              {images.map((img, i) => (
                <img key={i} src={img.url} alt="" className="w-14 h-14 rounded-lg object-cover border border-gray-100" />
              ))}
            </div>
          )}
          <div className="p-5">
            <p className="text-xs font-semibold uppercase tracking-wide" style={{ color: COLORS.primary }}>{categoryName}</p>
            <h2 className="text-xl font-bold text-gray-900 mt-1">{form.name}</h2>
            <p className="text-lg font-extrabold mt-1" style={{ color: COLORS.secondary }}>{money(Number(form.price))}</p>
            {form.location && <p className="text-sm text-gray-500 flex items-center gap-1 mt-2"><MapPin size={14} /> {form.location}</p>}
            {form.description && <p className="text-sm text-gray-600 mt-3 whitespace-pre-wrap">{form.description}</p>}
            <p className="text-xs text-gray-400 mt-3 capitalize">Condition: {form.condition}</p>
          </div>
        </div>

        <div className="mt-4 rounded-xl bg-amber-50 border border-amber-100 px-4 py-3 text-sm text-amber-800">
          After you confirm, this listing goes to SHINEX admin for review before it appears publicly. It will not go live immediately.
        </div>

        <div className="flex gap-3 mt-5">
          <Button variant="outline" className="flex-1 !bg-white" onClick={() => setStep("form")}>
            <ChevronLeft size={16} /> Edit
          </Button>
          <Button className="flex-1" onClick={submit} disabled={loading}>
            {loading ? "Submitting..." : editId ? "Save changes" : "Confirm & submit for review"}
          </Button>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold text-gray-900">{editId ? "Edit your listing" : "Sell something"}</h1>
      <p className="text-sm text-gray-500 mt-1 mb-6">
        {editId ? "Update the details below, then review your changes." : "List your item, preview it, then submit for review."}
      </p>
          ...
          <div className="flex gap-3 overflow-x-auto no-scrollbar pb-1">
            {images.length < 5 && (
              <button
                ...
                className="shrink-0 w-20 h-20 rounded-xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center gap-1 text-gray-400 hover:border-[#5B3FC6] hover:text-[#5B3FC6]"
              >
                <Plus size={18} />
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        ...
        {editing ? (
          <div className="mt-6 space-y-4">
            ...
            <Input label="WhatsApp number" value={form.whatsapp} onChange={(e) => setForm((f) => ({ ...f, whatsapp: e.target.value }))} placeholder="+2348012345678" />
            <p className="text-xs text-gray-400 -mt-2">Use international format, e.g. +2348012345678 — buyers tap this to message you on WhatsApp.</p>
          ...
          <div className="mt-6 space-y-3 text-sm">
            ...
            <div className="flex gap-3 pt-3">
              <Button variant="outline" className="!bg-white" onClick={() => go("myshop")}>Manage my shop</Button>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 mt-4 divide-y divide-gray-50 overflow-hidden">
        {[
          { label: "My shop", icon: Store, onClick: () => go("myshop") },
...
function AppShell() {
  ...
  switch (nav.page) {
    ...
    case "myshop": content = <MyShopPage go={go} />; break;
